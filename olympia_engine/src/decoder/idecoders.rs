...
use crate::decoder::{
    ...
    OneByteDecoder,
    TwoByteDataDecoder,
    TwoByteAddressDecoder,
    TwoByteOffsetDecoder,
    ThreeByteAddressDecoder,
    ThreeByteDataDecoder
};
...
fn byte_register_lookup(register_index: u8) -> Option<ByteRegisterLookupResult> {
    match register_index {
        0b000 => Some(ByteRegisterLookupResult::Register(registers::ByteRegister::B)),
        0b001 => Some(ByteRegisterLookupResult::Register(registers::ByteRegister::C)),
        0b010 => Some(ByteRegisterLookupResult::Register(registers::ByteRegister::D)),
        0b011 => Some(ByteRegisterLookupResult::Register(registers::ByteRegister::E)),
        0b100 => Some(ByteRegisterLookupResult::Register(registers::ByteRegister::H)),
        0b101 => Some(ByteRegisterLookupResult::Register(registers::ByteRegister::L)),
        0b110 => Some(ByteRegisterLookupResult::Memory),
        0b111 => Some(ByteRegisterLookupResult::Register(registers::ByteRegister::A)),
        _ => None
    }
}
...
impl TwoByteDataDecoder for LoadConstant {
    fn decode(&self, opcode: u8, data: u8) -> DecodeResult<Instruction> {
        ...
        match register {
            ...
            ByteRegisterLookupResult::Register(reg) => Ok(instructions::Load::Constant(reg, data).into())
        }
    }
}
...
impl TwoByteDataDecoder for Extended {
    fn decode(&self, _opcode: u8, data: u8) -> DecodeResult<Instruction> {
        ...
        if high_bits > 0 {
            ...
            let direction = if (data & 0b0000_1000) == 0 { Left } else { Right };
...
mod test {
    ...
    fn test_conditional_relative_jump() {
        ...
        assert_eq!(idecoder.decode(0x20, addr), Ok(RelativeJumpIf(cond::NonZero, addr).into()));
        assert_eq!(idecoder.decode(0x28, addr), Ok(RelativeJumpIf(cond::Zero, addr).into()));
        assert_eq!(idecoder.decode(0x30, addr), Ok(RelativeJumpIf(cond::NoCarry, addr).into()));
        assert_eq!(idecoder.decode(0x38, addr), Ok(RelativeJumpIf(cond::Carry, addr).into()));
    }
    ...
    fn test_load_constant_16() {
        ...
        assert_eq!(idecoder.decode(0x11, 0x2317), Ok(Load::Constant16(w::DE, 0x2317).into()));
        assert_eq!(idecoder.decode(0x21, 0x2350), Ok(Load::Constant16(w::HL, 0x2350).into()));
        assert_eq!(idecoder.decode(0x31, 0x2410), Ok(Load::Constant16(w::SP, 0x2410).into()));
    }
    ...
    fn test_indirect_a() {
        ...
        assert_eq!(idecoder.decode(0xEA, addr), Ok(instructions::Load::IndirectA(addr).into()));
    }
    ...
    fn test_a_indirect() {
        ...
        assert_eq!(idecoder.decode(0xFA, addr), Ok(instructions::Load::AIndirect(addr).into()));
    }
    ...
    fn test_jump() {
        ...
        assert_eq!(idecoder.decode(0xC2, addr), Ok(instructions::Jump::Jump(addr).into()));
    }
    ...
    fn test_call() {
        ...
        assert_eq!(idecoder.decode(0xC2, addr), Ok(instructions::Jump::Call(addr).into()));
    }
    ...
    fn test_conditional_jump() {
        ...
        assert_eq!(idecoder.decode(0xC2, addr), Ok(JumpIf(cond::NonZero, addr).into()));
        assert_eq!(idecoder.decode(0xCA, addr), Ok(JumpIf(cond::Zero, addr).into()));
        assert_eq!(idecoder.decode(0xD2, addr), Ok(JumpIf(cond::NoCarry, addr).into()));
        assert_eq!(idecoder.decode(0xDA, addr), Ok(JumpIf(cond::Carry, addr).into()));
    }
    ...
    fn test_conditional_call() {
        ...
        assert_eq!(idecoder.decode(0xC4, addr), Ok(CallIf(cond::NonZero, addr).into()));
        assert_eq!(idecoder.decode(0xCC, addr), Ok(CallIf(cond::Zero, addr).into()));
        assert_eq!(idecoder.decode(0xD4, addr), Ok(CallIf(cond::NoCarry, addr).into()));
        assert_eq!(idecoder.decode(0xDC, addr), Ok(CallIf(cond::Carry, addr).into()));
    }
    ...
    fn test_load_constant() {
        ...
        assert_eq!(idecoder.decode(0x06, 0x12), Ok(instructions::Load::Constant(b::B, 0x12).into()));
        assert_eq!(idecoder.decode(0x0E, 0x12), Ok(instructions::Load::Constant(b::C, 0x12).into()));
        assert_eq!(idecoder.decode(0x16, 0x12), Ok(instructions::Load::Constant(b::D, 0x12).into()));
        assert_eq!(idecoder.decode(0x1E, 0x12), Ok(instructions::Load::Constant(b::E, 0x12).into()));
        assert_eq!(idecoder.decode(0x26, 0x12), Ok(instructions::Load::Constant(b::H, 0x12).into()));
        assert_eq!(idecoder.decode(0x2E, 0x12), Ok(instructions::Load::Constant(b::L, 0x12).into()));
        assert_eq!(idecoder.decode(0x36, 0x24), Ok(instructions::Load::ConstantMemory(0x24).into()));
        assert_eq!(idecoder.decode(0x3E, 0x12), Ok(instructions::Load::Constant(b::A, 0x12).into()));
    }
    ...
    fn test_constant_al() {
        ...
        assert_eq!(idecoder.decode(0xC6, 0x12), Ok(Instruction::ConstantAL(op::Add, 0x12)));
        assert_eq!(idecoder.decode(0xCE, 0x12), Ok(Instruction::ConstantAL(op::AddCarry, 0x12)));
        assert_eq!(idecoder.decode(0xD6, 0x12), Ok(Instruction::ConstantAL(op::Sub, 0x12)));
        assert_eq!(idecoder.decode(0xDE, 0x12), Ok(Instruction::ConstantAL(op::SubCarry, 0x12)));
        assert_eq!(idecoder.decode(0xE6, 0x12), Ok(Instruction::ConstantAL(op::And, 0x12)));
        assert_eq!(idecoder.decode(0xEE, 0x12), Ok(Instruction::ConstantAL(op::Xor, 0x12)));
        assert_eq!(idecoder.decode(0xF6, 0x12), Ok(Instruction::ConstantAL(op::Or, 0x12)));
        assert_eq!(idecoder.decode(0xFE, 0x12), Ok(Instruction::ConstantAL(op::Compare, 0x12)));
    }
    ...
    fn test_store_high_offset() {
        ...
        assert_eq!(idecoder.decode(0xE0, 0x55.into()), Ok(instructions::Load::AHighOffset(0x55.into()).into()));
    }
    ...
    fn test_load_high_offset() {
        ...
        assert_eq!(idecoder.decode(0xF0, 0x55.into()), Ok(instructions::Load::HighOffsetA(0x55.into()).into()));
    }
    ...
    fn test_add_stack_pointer() {
        ...
        assert_eq!(idecoder.decode(0xE8, 0x55.into()), Ok(instructions::Stack::AddStackPointer(0x55.into()).into()));
    }
    ...
    fn test_calc_stack_offset() {
        ...
        assert_eq!(idecoder.decode(0xF8, 0x55.into()), Ok(instructions::Stack::LoadStackOffset(0x55.into()).into()));
    }
    ...
    fn test_extended_rotate() {
        ...
        assert_eq!(idecoder.decode(0xCB, 0x01), Ok(Extended::Rotate(Left, Carry, b::C).into()));
        assert_eq!(idecoder.decode(0xCB, 0x14), Ok(Extended::Rotate(Left, NoCarry, b::H).into()));
        assert_eq!(idecoder.decode(0xCB, 0x0D), Ok(Extended::Rotate(Right, Carry, b::L).into()));
        assert_eq!(idecoder.decode(0xCB, 0x1F), Ok(Extended::Rotate(Right, NoCarry, b::A).into()));

        assert_eq!(idecoder.decode(0xCB, 0x06), Ok(Extended::RotateMemory(Left, Carry).into()));
        assert_eq!(idecoder.decode(0xCB, 0x16), Ok(Extended::RotateMemory(Left, NoCarry).into()));
        assert_eq!(idecoder.decode(0xCB, 0x0E), Ok(Extended::RotateMemory(Right, Carry).into()));
        assert_eq!(idecoder.decode(0xCB, 0x1E), Ok(Extended::RotateMemory(Right, NoCarry).into()));
    }
    ...
    fn test_extended_shift() {
        ...
        assert_eq!(idecoder.decode(0xCB, 0x21), Ok(Extended::ShiftHigh(Left, b::C).into()));
        assert_eq!(idecoder.decode(0xCB, 0x2F), Ok(Extended::ShiftHigh(Right, b::A).into()));
        assert_eq!(idecoder.decode(0xCB, 0x3B), Ok(Extended::ShiftRightZero(b::E).into()));

        assert_eq!(idecoder.decode(0xCB, 0x26), Ok(Extended::ShiftMemoryHigh(Left).into()));
        assert_eq!(idecoder.decode(0xCB, 0x2E), Ok(Extended::ShiftMemoryHigh(Right).into()));
        assert_eq!(idecoder.decode(0xCB, 0x3E), Ok(Extended::ShiftMemoryRightZero.into()));
    }
    ...
    fn test_extended_test_bit() {
        ...
        assert_eq!(idecoder.decode(0xCB, 0x74), Ok(Extended::TestBit(6, b::H).into()));
        assert_eq!(idecoder.decode(0xCB, 0x58), Ok(Extended::TestBit(3, b::B).into()));
        assert_eq!(idecoder.decode(0xCB, 0x6E), Ok(Extended::TestMemoryBit(5).into()));
    }
    ...
    fn test_extended_reset_bit() {
        ...
        assert_eq!(idecoder.decode(0xCB, 0x80), Ok(Extended::ResetBit(0, b::B).into()));
        assert_eq!(idecoder.decode(0xCB, 0x93), Ok(Extended::ResetBit(2, b::E).into()));
        assert_eq!(idecoder.decode(0xCB, 0x8E), Ok(Extended::ResetMemoryBit(1).into()));
    }
    ...
    fn test_extended_set_bit() {
        ...
        assert_eq!(idecoder.decode(0xCB, 0xC4), Ok(Extended::SetBit(0, b::H).into()));
        assert_eq!(idecoder.decode(0xCB, 0xD5), Ok(Extended::SetBit(2, b::L).into()));
        assert_eq!(idecoder.decode(0xCB, 0xDE), Ok(Extended::SetMemoryBit(3).into()));
    }
}